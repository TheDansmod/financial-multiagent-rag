...
def explore_vector_db(cfg):
    ...
    points = client.scroll(collection_name=coll_name, limit=1, with_payload=['metadata'], with_vectors=False)
    # points[0] will have length = limit, each element is a Record, each record has record.payload, each payload is a dictionary which has key metadata. The value of the metadata key is also a dictionary with contains Header N (maybe only some N), company, and year.
    print(points)


class QueryType(TypedDict):
    query_type: Literal["simple", "comparison", "complex"]
    query_companies: list[Literal["NVDA", "AAPL", "MSFT", "AMZN", "GOOGL", "META", "TSLA", "WMT", "JPM", "NFLX"]] | None

class SubQueries(TypedDict):
    sub_queries: list[str]
    company: list[Literal["NVDA", "AAPL", "MSFT", "AMZN", "GOOGL", "META", "TSLA", "WMT", "JPM", "NFLX"]]
...
def test_gemini_flash(cfg):
    from langchain_google_genai import ChatGoogleGenerativeAI as Gemini

    model = Gemini(model=cfg.model.name, temperature=cfg.model.temp)
    prompt = "When using the oil plugin on neovim, I have added the option show_hidden = true in the view_options section for the plugin, but when I close neovim and re-open it, the hidden files are still not visible. How to fix this issue?"
...
def analyse_headers(cfg):
    """Figure out what all the headers are like, for all the files you have.

    We have figured out the following so far:
    ```
        # number of points in the collection = 7687
...
if __name__ == "__main__":
    ...
    complex_query = "What is the primary risk factor listed by the company with the highest revenue in the tech sector?"
    analyse_headers(cfg)
