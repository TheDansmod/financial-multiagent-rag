...
class QueryType(TypedDict):
    ...
    query_companies: list[Literal["NVDA", "AAPL", "MSFT", "AMZN", "GOOGL", "META", "TSLA", "WMT", "JPM", "NFLX"]]
...
class FinancialAnalystAgent:
    ...
    def __init__(self, cfg):
        model_provider = initialize(cfg.model)
        self.model = model_provider(model=cfg.model.name, temperature=cfg.model.temp)
        self.cfg = cfg

    def supervisor(self, state: AgentState) -> Command[Literal["planner", "retriever"]]:
        template = self.cfg.agent_configs.supervisor_node.system_prompt_template
